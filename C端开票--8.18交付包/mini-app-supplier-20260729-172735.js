(function () {
  ...
  function formatMiniDemoTime(offsetMinutes) {
  ...
  function formatMiniDemoStamp(offsetMinutes) {
    var current = new Date(Date.now() - (parseInt(offsetMinutes, 10) || 0) * 60000);
  ...
  var afterSaleRuntime = (window.__MINI_AFTER_SALE_RUNTIME__ = window.__MINI_AFTER_SALE_RUNTIME__ || {
    ...
    orders: [
      ...
      },
      {
        ...
        uploadedFile: {
          name: "invoice_starbucks_fail_" + formatMiniDemoStamp(2) + ".pdf",
          time: formatMiniDemoTime(2),
          chat: "星巴克售后群"
        },
        rejectReason: "",
        removed: false
      },
      {
        ...
        uploadedFile: {
          name: "invoice_kfc_fail_" + formatMiniDemoStamp(3) + ".pdf",
          time: formatMiniDemoTime(3),
          chat: "肯德基工单沟通群"
        },
        rejectReason: "",
        removed: false
      },
      {
        ...
        uploadedFile: {
          name: "invoice_reject_fail_" + formatMiniDemoStamp(4) + ".pdf",
          time: formatMiniDemoTime(4),
          chat: "麦当劳补贴单群"
        },
      ...
      },
      {
        ...
        uploadedFile: {
          name: "invoice_upload_pass_" + formatMiniDemoStamp(5) + ".pdf",
          time: formatMiniDemoTime(5),
          chat: "塔斯汀售后群"
        },
        rejectReason: "",
        removed: false
    ...
    chats: [
      {
        ...
        desc: "最近上传 2 个文件",
        files: [
          { name: "invoice_luckin_pass_" + formatMiniDemoStamp(0) + ".pdf", time: formatMiniDemoTime(0) },
          { name: "invoice_luckin_fail_" + formatMiniDemoStamp(1) + ".ofd", time: formatMiniDemoTime(1) }
        ]
      },
      {
        id: "chat-chagee",
        ...
        desc: "最近上传 2 个文件",
        files: [
          { name: "invoice_chagee_pass_" + formatMiniDemoStamp(6) + ".pdf", time: formatMiniDemoTime(6) },
          { name: "invoice_chagee_fail_" + formatMiniDemoStamp(7) + ".pdf", time: formatMiniDemoTime(7) }
        ]
      },
      {
        id: "chat-kfc",
        ...
        desc: "最近上传 2 个文件",
        files: [
          { name: "invoice_kfc_pass_" + formatMiniDemoStamp(8) + ".ofd", time: formatMiniDemoTime(8) },
          { name: "invoice_kfc_fail_" + formatMiniDemoStamp(9) + ".pdf", time: formatMiniDemoTime(9) }
        ]
      }
    ]
  });
  ...
  function getAutoAuditOutcome(fileName) {
    var normalized = String(fileName || "").toLowerCase();
    if (normalized.indexOf("pass") >= 0) return "pass";
    if (normalized.indexOf("fail") >= 0) return "fail";
    return "";
  }
  ...
  function renderUploadModal(host) {
    return (
      ...
      "</div>" +
      '<div style="margin:0 0 14px;padding:12px 14px;border:1px solid #dbe7fb;border-radius:16px;background:#f8fbff;">' +
      '<ul class="text" spellcheck="false" style="margin:0;padding-left:18px;color:#1f3b6d;font-size:12px;line-height:1.7;font-weight:600;">' +
  ...
  function renderFileModal(host) {
    return (
      ...
      host +
      '" data-app="wps"><div class="text" spellcheck="false">WPS查看</div></button>' +
      '<button type="button" class="ghost-btn ax_default mini-after-sale-file-open" data-host="' +
      host +
      '" data-app="browser" style="background:#1d4ed8;border-color:#1d4ed8;"><div class="text" spellcheck="false" style="color:#fff;">浏览器查看</div></button>' +
      "</div>" +
      "</div>" +
      "</div>"
    );
  ...
  function getListActions(order) {
    ...
    if (order.status === "rejected") {
      return [
        { label: "查看", action: "view-file", primary: false },
        { label: "重新上传", action: "reupload", primary: true }
      ];
    }
  ...
  function renderUploadedFileLine(uploadModal, order, host) {
    ...
    if (!line) return;
    if (afterSaleRuntime.currentAction === "reupload") {
      line.innerHTML = "";
      return;
    }
  ...
  function openFileInBrowser(order, appType) {
    if (!order || !order.uploadedFile) return;
    var appName = appType === "wps" ? "WPS" : "浏览器";
    var appTag = appType === "wps" ? "WPS查看示意" : "浏览器查看示意";
    ...
    var previewHtml =
      ...
      "</p><p>打开方式：" +
      appName +
      "</p><p>该原型用于演示：点击“查看”后，可选择 WPS、浏览器等软件查看已上传文件。</p></div></body></html>";
  ...
  function submitUpload(host) {
    ...
    var outcome = getAutoAuditOutcome(order.uploadedFile && order.uploadedFile.name);
    afterSaleRuntime.uploadProgressHost = "";
    afterSaleRuntime.uploadProgressValue = 0;
    window.clearTimeout(afterSaleRuntime.uploadProgressTimer);
    window.clearTimeout(afterSaleRuntime.uploadProgressDoneTimer);
    if (!outcome) {
      renderAfterSaleLists();
      renderDetailPage();
      showToast("文件名需包含 pass 或 fail，才能模拟系统自动审核结果");
      return;
    }
    showToast("上传成功，系统自动审核中...");
    window.setTimeout(function () {
      order.status = outcome === "pass" ? "uploaded" : "reviewing";
      order.deadline = "";
      if (outcome === "pass") order.rejectReason = "";
      closeAllAfterSaleModals();
      afterSaleRuntime.currentTab = outcome === "pass" ? "uploaded" : "reviewing";
      renderAfterSaleLists();
      renderDetailPage();
      showBundlePage("mini-after-sale");
      showToast(outcome === "pass" ? "系统自动审核通过" : "系统自动审核失败，已进入审核中");
    }, 700);
  }
  ...
  function bindMiniAfterSaleEvents() {
    document.addEventListener(
      ...
      function (event) {
        ...
        if (fileOpen) {
          event.preventDefault();
          openFileInBrowser(getOrderById(afterSaleRuntime.currentOrderId), fileOpen.getAttribute("data-app") || "browser");
          return;
        }
